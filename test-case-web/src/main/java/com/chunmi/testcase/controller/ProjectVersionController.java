...
public class ProjectVersionController {
	...
	@Resource
	private ProjectVersionService projectVersionServiceImpl;
	...
	public String projectVersionList(HttpServletRequest request,Model model,@PathVariable("pageCurrent") Integer pageCurrent,
		...
		try {
			PageBean<ProjectVersion> pb = projectVersionServiceImpl.selectProjectVersionByCondition(projectVersion,pageCurrent,pageSize,pageCount);
			map.put("pb",pb);
			//生成新的查询url
	...
	public Response addProjectVersion(ProjectVersion projectVersion) {
		try {
			if(projectVersionServiceImpl.seletProjectVersionByProjectIdAndVersionNum(projectVersion)!=null)
				return Response.getError(MessageExceptionEnum.VERSION_EXISTED);
			projectVersionServiceImpl.addProjectVersion(projectVersion);
